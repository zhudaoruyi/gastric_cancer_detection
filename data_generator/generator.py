    ...
    while True:
        ...
        for i in range(batch_size):
            if (i % 2):  # 取到圈出的部分(含cancer)
                ...
                img = img.crop((random_x, random_y, random_x + widths, random_y + heights))
                # 数据增强部分
                if train_set:
                    img_r = img.rotate(np.random.random(1)*360.)
                    img = nd.array(img_r)
...
#                img3 = np.array(img2.convert("RGB")) / 255.
                ...
                img = slide.crop((random_x, random_y, random_x + widths, random_y + heights))
                # 数据增强部分
                if train_set:
                    img_r = img.rotate(np.random.random(1)*360.)
                    img = nd.array(img_r)
